...
def create_uniform_ate(dag: DAG, interval: tuple = None, width: float = 1e-2):
    ...
    def param_fn(z, generator, device, data):
        if interval is None:
            ...
            rng = interval
        treatments0 = torch.rand(size=(z.shape[0], 1)) * (2*width) + rng[0]-width
        cntf0 = generator.do(z, treatments0.to(device), dag.do_var, data=data)[:, target_start:target_end]
        treatments1 = torch.rand(size=(z.shape[0], 1)) * (2*width) + rng[1]-width
        cntf1 = generator.do(z, treatments1.to(device), dag.do_var, data=data)[:, target_start:target_end]
        return cntf1-cntf0
