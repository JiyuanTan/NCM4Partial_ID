...
def acic_noisy(noise_level: int, n_samples: int, max_epochs: int, seed: int, gpus: int = 1, num_workers: int = 4,
               n_hidden: int = 64, n_layers: int = 3, lr: float = 0.001, lagrange_lr: float = 0.5):
    """Run ACIC experiment with noisy measurement.
    Args:
        noise_level (int):
        0-5, choose the noise from [no noise, N(0.1, 0.5), N(0.2, 0.5), N(0.3, 1), N(0.4, 1), N(0.5, 1)]
    """
